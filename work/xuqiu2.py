...
def login(username,password):
    ...
    # 用户名或密码不匹配的情况
    result.update({"code":1,"message":"请检查用户名或密码是否填写正确。"})
    return result

# 用户查询功能
def get_usre(username):


     #判断用户名是否登录，若登录成功可进行查询，若登录失败，返回权限不足
